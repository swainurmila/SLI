            ...
            <div id="panel-1" class="panel">
                <div class="panel-hdr">
                    ...
                    <div class="panel-toolbar">
                        ...
                        <a href="{{route("$routeName.create",[$routeName,$id])}}" class="btn btn-success waves-effect waves-themed">Create</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($postType as $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{-- {{route('temp_destroy',[$item->id])}} --}}
                                            <a href="{{route("$routeName.delete",[$routeName,$item->id])}}"
                                                class="btn btn-sm btn-icon btn-outline-danger rounded-circle mr-1"
                                                title="Delete Record">
                                                <i class="fad fa-trash-restore-alt"></i>
                                            </a>
                                            <a href="{{route("$routeName.edit",[$routeName,$item->id])}}"
