            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0">Assign classes for {{@$traner_user_data->first_name}}</h4>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($classes as $class)
                                            <tr class="text-center">
                                                <td>{{ ++$i }}</td>
                                                <?php 
                                                 $training_data =  App\Models\Training\TrTraining::where('id', $class->training_id)->first();

                                                 
                                                 $training_batch_ditails =  App\Models\Training\TrTrainingDetail::where('id', $class->training_details_id)->first();
                                                ?>
                                                <td>{{@$training_data->name}}</td>

                                                
                                                <td>{{@$training_data->training_duration}} - {{@$training_data->training_duration_type}}</td>
                                                
                                                <td>{{ date('d-m-Y',strtotime(@$training_batch_ditails->start_date)) }} </td>
                                                <td> {{ date('d-m-Y',strtotime(@$training_batch_ditails->end_date)) }}       </td>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ route('training.admin.about', ['id' => @$class->training_id]) }}" class="btn custom-btn btn-sm edit waves-effect waves-light">About
                                                        Training</a>

                                                        <a href="{{ route('attendance.index', ['id' => @$class->id]) }}" class="btn btn-danger btn-sm edit waves-effect waves-light">
                                                            Attendance</a>
                                                </td>
                                            </tr>
                                        @endforeach
