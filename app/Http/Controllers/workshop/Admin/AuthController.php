    ...
    public function attemptLogin(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($user && Hash::check($request->password, $user->password)) {
                if ($user->status == 1 && $user->role_id == 10 && $user->is_workshop == 1) {
                    // Attempt to log in the training
                    if (Auth::attempt($credentials, $request->filled('remember'))) {
                        //return 1;
                        return redirect()->route('workshop.admin.dashboard')
                            ->withSuccess('You have Successfully Logged in');
                    } else {
                        Session::flash('error', 'Oppes ! You have entered invalid credentials');
                        return redirect()->back();
                    }
                } else if ($user->status == 1 && $user->role_id == 5 && $user->is_workshop == 1) {
                    if (Auth::attempt($credentials, $request->filled('remember'))) {
                        return redirect()->route('workshop.user.dashboard')
                            ->withSuccess('You have Successfully Logged in');
                    } else {
                        Session::flash('error', 'Oppes ! You have entered invalid credentials');
                        return redirect()->back();
