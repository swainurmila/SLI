...
class SCHEMESANDSERVICESController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = SCHEMESANDSERVICES::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.schemes_and_services.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.schemes_and_services.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType = new SCHEMESANDSERVICES();
        ...
        $PostType->save();
        return redirect()->route('schemes-and-services.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = SCHEMESANDSERVICES::find($id);
        return view("admin.post_types.schemes_and_services.edit",compact('lang','postType','id','postTypeName'));
    }
