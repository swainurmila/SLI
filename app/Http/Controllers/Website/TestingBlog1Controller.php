...
use App\Models\Website\TestingBlog1;
...
class TestingBlog1Controller extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = TestingBlog1::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.testing_blog1.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.testing_blog1.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('testing-blog1.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = TestingBlog1::find($id);
        return view("admin.post_types.testing_blog1.edit",compact('lang','postType','id','postTypeName'));
    }
