...
use App\Models\Website\BlogTesting;
...
class BlogTestingController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = BlogTesting::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.blog_testing.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.blog_testing.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('blog-testing.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = BlogTesting::find($id);
        return view("admin.post_types.blog_testing.edit",compact('lang','postType','id','postTypeName'));
    }
