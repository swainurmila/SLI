...
class QuickLinksController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        ...
        $postType = QuickLinks::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.quicklinks.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.quicklinks.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('quicklinks.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {

        $lang = Language::all();
        $postType = QuickLinks::find($id);
        return view("admin.post_types.quicklinks.edit",compact('lang','postType','id','postTypeName'));
    }
