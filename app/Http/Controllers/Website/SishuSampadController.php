...
use App\Models\Website\SishuSampad;
...
class SishuSampadController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = SishuSampad::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.sishu_sampad.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.sishu_sampad.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('sishu-sampad.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = SishuSampad::find($id);
        return view("admin.post_types.sishu_sampad.edit",compact('lang','postType','id','postTypeName'));
    }
