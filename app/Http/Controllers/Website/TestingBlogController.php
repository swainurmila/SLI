...
use App\Models\Website\TestingBlog;
...
class TestingBlogController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = TestingBlog::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.testing_blog.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.testing_blog.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('testing-blog.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = TestingBlog::find($id);
        return view("admin.post_types.testing_blog.edit",compact('lang','postType','id','postTypeName'));
    }
