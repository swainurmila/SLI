...
use App\Models\Website\ShramaDarpan;
...
class ShramaDarpanController extends Controller
{
    public function index(Request $request, $postTypeName,$id)
    {
        $postType = ShramaDarpan::all();
        $routeName = $postTypeName;
        //return view('admin.post_types.shrama_darpan.index',compact('postType','routeName'));
    ...
    public function create($postTypeName,$id)
    {
        ...
        $lang = Language::all();
        $routeName = $postTypeName;
        return view('admin.post_types.shrama_darpan.addPage',compact('lang','routeName','id'));
    }
    public function store(Request $request)
    {
        ...
        $PostType->save();
        return redirect()->route('shrama-darpan.index',[$request->post_type,$request->post_id]);
    }
    public function edit(Request $request,$postTypeName,$id)
    {
        $lang = Language::all();
        $postType = ShramaDarpan::find($id);
        return view("admin.post_types.shrama_darpan.edit",compact('lang','postType','id','postTypeName'));
    }
